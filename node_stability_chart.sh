...
tmp="$(mktemp -d)"
#[ -n "$tmp" ] && [ -d "$tmp" ] && trap "rm -r '$tmp'" EXIT
...
uv run circleci_explore.py --slug gh/cerebrotech/domino --build "$build_id" --pipeline-summary > "$tmp/pipeline.json"

start_time="$(jq -r '.pipeline.workflows[].jobs[] | select(.name | contains("create-deployment")) | .end' "$tmp/pipeline.json" | sed -e 's/T/ /g' -e 's/Z$/+0000/g')"
end_time="$(jq -r '.pipeline.end' "$tmp/pipeline.json" | sed -e 's/T/ /g' -e 's/Z$/+0000/g')"

uv run newrelic_chart_dashboard.py --image-out "/tmp/node-stability-$stagename.png" --visualization viz.stacked-bar --title "Node stability of $stagename" <<EOF
